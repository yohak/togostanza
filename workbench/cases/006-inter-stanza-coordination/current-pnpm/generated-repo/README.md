...
Case input project for observing current-version inter-stanza coordination behavior.
...
## Case input and observation helper structure
...
- `fixtures/inter-stanza.html` wires the observation helper with `togostanza--container`, `togostanza--event-map`, and `togostanza--data-source`.
...
- `togostanza--data-container` is intentionally not present in this observation helper because the verification case treats it as an old documentation typo, not as a current custom element to preserve.
