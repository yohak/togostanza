...
- 出力先が空でなく、現在のリメイク版が管理するbuild生成物として識別できない場合、非対話環境では自動clearしないこと。
- 出力先が空でなく、現在のリメイク版が管理するbuild生成物として識別できない場合、対話可能な端末ではwarning後にclear / abortを選べること。
...
- 非対話環境では、未識別の非空出力先を自動削除しない。
- 対話確認でclearを選んだ場合だけ、未識別の非空出力先を削除してbuildできる。
...
- ビルド時の警告/エラー。
- 未識別の非空出力先に対するwarning、対話確認、非対話時の拒否。
