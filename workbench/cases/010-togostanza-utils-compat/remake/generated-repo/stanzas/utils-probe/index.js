...
export default class UtilsProbe extends Stanza {
  ...
  async render() {
    ...
    const jsonData = await loadData(this.params["json-url"], "json", main);
    ...
    appendCustomCss(this, this.params["custom-css-a-url"]);
    appendCustomCss(this, this.params["custom-css-b-url"]);
    const customLinks = this.root.querySelectorAll("link[data-togostanza-custom-css]");
    checks.push(
      assertCheck(
        "appendCustomCss replacement",
        customLinks.length === 1 && customLinks[0].href === this.params["custom-css-b-url"],
