...
const previewDataElement = document.querySelector("#togostanza-preview-data");
const mountElement = document.querySelector("#togostanza-help-app");
if (!previewDataElement || !mountElement) {
    throw new Error("TogoStanza help preview could not find its page data or mount element.");
}
const previewData = JSON.parse(previewDataElement.textContent ?? "");
...
createApp({
    setup() {
        ...
        const parameterValues = reactive(Object.fromEntries(parameterFields.map((field) => [field.key, field.defaultValue])));
        const styleValues = reactive(Object.fromEntries(styleFields.map((field) => [field.key, field.defaultValue])));
        const parameterTouched = reactive({});
        const styleTouched = reactive({});
        ...
        const previewStyles = computed(() => Object.fromEntries(styleFields
            .filter((field) => field.hasInitialValue || styleTouched[field.key])
            .map((field) => [field.key, styleValues[field.key] ?? ""])));
        const snippet = computed(() => formatSnippet(previewData.id, parameterFields, parameterValues, parameterTouched, styleFields, styleValues, styleTouched));
        ...
        return () => h("main", { class: "container-fluid py-4 px-lg-5" }, [
            h("header", { class: "mb-4" }, [
            ...
            h("div", { class: "row g-4 align-items-start" }, [
                h("section", { class: "col-xl-5" }, [
                    ...
                    h("div", { class: "border border-top-0 rounded-bottom p-3 p-lg-4 bg-body" }, renderActivePanel(activeTab.value, parameterFields, parameterValues, parameterTouched, styleFields, styleValues, styleTouched, previewData)),
                ]),
...
function renderTabs(activeTab) {
    ...
    return h("nav", { "aria-label": "Stanza help sections", class: "nav nav-tabs" }, tabs.map(([id, label]) => h("button", {
...
function renderActivePanel(activeTab, parameterFields, parameterValues, parameterTouched, styleFields, styleValues, styleTouched, data) {
    switch (activeTab) {
        case "parameters":
...
function renderFields(fields, values, touched, kind) {
    if (fields.length === 0) {
        return h("p", { class: "text-body-secondary mb-0" }, `No ${kind}s are defined.`);
    }
    return fields.map((field) => h("div", { class: "mb-4", key: field.key }, [
...
function renderFieldInput(field, values, touched, kind) {
    ...
    if (field.type === "boolean") {
        return h("div", { class: "form-check form-switch" }, [
            h("input", {
                ...
                onChange: (event) => {
                    touched[field.key] = true;
                    values[field.key] = String(event.target.checked);
                },
                role: "switch",
                type: "checkbox",
            }),
            h("label", { class: "form-check-label", for: id }, booleanValue(values[field.key] ?? "") ? "Enabled" : "Disabled"),
        ]);
    }
...
function readFields(value, kind) {
...
function readMetadataEntries(value) {
    return Array.isArray(value) ? value.filter(isRecord) : [];
}
function formatSnippet(id, parameterFields, parameterValues, parameterTouched, styleFields, styleValues, styleTouched) {
    ...
    const changedStyles = styleFields.filter((field) => (field.hasInitialValue || styleTouched[field.key]) &&
        (styleValues[field.key] ?? "") !== field.defaultValue);
    const styleBlock = changedStyles.length === 0
        ? []
        : [
            "<style>",
            `  ${tagName} {`,
...
function isDisplayableMetadataValue(value) {
