import { copyFileSync, existsSync, mkdtempSync, mkdirSync, readdirSync, readFileSync, realpathSync, rmSync, statSync, writeFileSync, } from "node:fs";
...
export async function handleBuild(args, options = {}) {
...
export async function buildStanzaArtifacts(input) {
    ...
    try {
        ...
        await buildEntrypoints(selectedStanzaResult.stanzas, input.outputDirectory, input.rootDirectory, buildConfigResult.config.vite);
        ...
        writeHtmlFiles(selectedStanzaResult.stanzas, stanzaResult.stanzas, input.outputDirectory, input.rootDirectory);
    }
    catch (error) {
...
function resolveOutputDirectory(rootDirectory, outputPath) {
    if (outputPath.trim() === "") {
...
function discoverStanzas(rootDirectory) {
...
function selectStanzas(stanzas, stanzaIds) {
    if (!stanzaIds) {
...
function resolveEntrypoint(stanzaDirectory, stanzaId) {
...
function outputDirectoryCleanStatus(outputDirectory, rootDirectory) {
...
function outputDirectoryRealpathStatus(outputDirectory, rootDirectory) {
...
function canonicalizeThroughExistingAncestor(path) {
    let candidate = path;
    const missingParts = [];
...
function prepareOutputDirectory(outputDirectory, rootDirectory, options) {
...
async function resolveOutputDirectoryCleanPermission(input) {
    ...
    try {
        const outputDirectoryStatus = outputDirectoryCleanStatus(input.outputDirectory, input.rootDirectory);
        if ("error" in outputDirectoryStatus) {
...
async function confirmCleanOutputInteractively(input) {
    ...
    try {
        const answer = await readline.question(`${input.warning}\nClear and overwrite this directory? [y/N] `);
...
function writeOutputMarker(outputDirectory) {
    writeFileSync(join(outputDirectory, outputMarkerFileName), "Generated by togostanza build.\n", "utf8");
}
async function buildEntrypoints(stanzas, outputDirectory, rootDirectory, userViteConfig) {
    const wrapperDirectory = mkdtempSync(join(tmpdir(), "togostanza-build-"));
    const input = Object.fromEntries(stanzas.map((stanza) => {
...
async function buildHelpPreviewApp(outputDirectory) {
...
function collectEntryCssPlugin(outputDirectory) {
    const entryCssByName = new Map();
    return {
...
function readViteImportedCss(output) {
    if (!isRecord(output)) {
        return new Set();
    }
...
function formatEntrypointWrapper(stanza) {
...
function buildStyles(stanzas, outputDirectory, rootDirectory) {
    for (const stanza of stanzas) {
        ...
        try {
            const result = compileString(preprocessSassSource(readFileSync(stylePath, "utf8"), rootDirectory), {
            ...
            throw new Error(`Sass compile failed for ${stanza.id} at ${stylePath}: ${errorMessage(error)}`, { cause: error });
        }
    }
}
function rewriteStanzaAssetUrls(css, stanzaId) {
    return css.replace(/url\(\s*(["']?)(\.\/assets\/|assets\/)([^"')\s]+)\1\s*\)/g, (_match, quote, _prefix, assetPath) => `url(${quote}./${stanzaId}/assets/${assetPath}${quote})`);
}
function preprocessSassSource(source, rootDirectory) {
    return source.replace(/(@(?:import|use|forward)\s+)(["'])\.\/(@[^"']+)\2/g, (_match, directive, quote, packagePath) => {
        const resolvedPath = join(rootDirectory, "node_modules", packagePath);
        return `${directive}${quote}${resolvedPath}${quote}`;
    });
}
function readGeneratedCss(cssPath, stanzaId) {
    if (!pathIsFile(cssPath)) {
        return "";
    }
    return readFileSync(cssPath, "utf8")
        .replace(new RegExp(String.raw `/\*# sourceMappingURL=${escapeRegExp(stanzaId)}\.css\.map \*/`, "g"), "")
        .trim();
}
function formatGeneratedCss(parts, sourceMapFileName) {
    const css = parts
        .map((part) => part.trim())
...
function copyBuildAssets(stanzas, outputDirectory, rootDirectory, options = {}) {
    if (options.copyRootAssets ?? true) {
        copyDirectoryContents(join(rootDirectory, "assets"), join(outputDirectory, "assets"));
    }
    for (const stanza of stanzas) {
        copyDirectoryContents(join(stanza.directory, "assets"), join(outputDirectory, stanza.id, "assets"));
...
function writeHtmlFiles(builtStanzas, allStanzas, outputDirectory, rootDirectory) {
    writeFileSync(join(outputDirectory, "index.html"), formatIndexHtml(allStanzas, readRepositoryName(rootDirectory)), "utf8");
    for (const stanza of builtStanzas) {
        writeFileSync(join(outputDirectory, `${stanza.id}.html`), formatHelpHtml(stanza), "utf8");
    }
}
function formatIndexHtml(stanzas, repositoryName) {
...
function formatHelpHtml(stanza) {
...
function readRepositoryName(rootDirectory) {
    try {
        const packageValue = JSON.parse(readFileSync(join(rootDirectory, "package.json"), "utf8"));
...
function createSassImporter(rootDirectory) {
    return {
        canonicalize(url) {
            if (url.startsWith("@/")) {
            ...
            if (url.startsWith("./@")) {
                return pathToFileURL(resolveSassImportPath(join(rootDirectory, "node_modules", url.slice(2))));
            }
            return null;
        },
        load(canonicalUrl) {
...
function resolveSassImportPath(pathWithoutExtension) {
    const candidates = [
...
function handlebarsRuntimePath() {
...
function readTemplates(stanzaDirectory, stanzaId) {
...
function formatTemplates(templates) {
    ...
    return [
        "{",
        ...entries.map(([name, template]) => `  ${JSON.stringify(name)}: Handlebars.template(${template}),`),
        "}",
    ].join("\n");
}
...
function pathIsInsideOrEqual(rootDirectory, candidatePath) {
...
function formatBuildError(error, rootDirectory) {
    const message = errorMessage(error);
    const hint = isUnresolvedImportError(message) && tsconfigLikelyDefinesPaths(rootDirectory)
...
function isUnresolvedImportError(message) {
    return (message.includes("failed to resolve import") ||
        message.includes("Could not resolve") ||
        message.includes("Could not load"));
}
function tsconfigLikelyDefinesPaths(rootDirectory) {
    try {
