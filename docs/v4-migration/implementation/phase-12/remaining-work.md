...
| タグ無しGitHub dependencyのlockfile更新手順 | local smokeでは、lockfileなしfresh install、lockfileありfrozen install、同じdependency specでの明示更新を確認する。Stanza開発者向けの短い更新手順は `docs/v4-migration/guides/github-dependency.md` に記録済み。 | documented constraint | `main` 更新後に既存Stanzaリポジトリがいつ・どう追従するかに影響する。 | 実利用で不足が見えたら、生成README側にも要約を追加する。 |
...
| リッチなヘルププレビュー | Phase 13で、`build` と `serve` が `index.html`、`{id}.html`、ヘルププレビューappを生成・配信するようにした。パラメーター変更UI、style変更UI、HTML snippet、About、Download JSON導線を確認済み。 | 完了済み | Stanza開発者がローカル確認や公開済み生成物からstanzaを調整する導線は復元された。 | 残る改善はREADME本文表示、JSON専用editor、イベント操作UIとして別項目で扱う。 |
| menu UI polish | Phase 14で、`<togostanza--menu>` custom element、info icon、popup、Copy HTML snippet、About link、`this.menu()` item / divider、`togostanza-utils` のdownload系menu helper確認を実装・確認済み。現行版に近づけるため、popupの文字サイズ、button装飾、横overflow、buttonとの位置関係も調整した。 | 完了済み | 埋め込み後のmenu挙動は、Stanza利用者とStanza開発者の体験に直接影響する。 | 残る細部は、実利用で劣化が見えた場合に個別改善として扱う。 |
