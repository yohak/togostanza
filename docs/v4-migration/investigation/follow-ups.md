...
## init時のJavaScript / TypeScript選択

- 種別: Beta移行に向けた後続機能。ユーザー指定の残タスクとして追加。
...
- 選択に応じた `index.js` / `index.ts` の生成方法と、既存の生成手順への組み込み。
- TypeScript選択時に用意する `tsconfig.json` と開発依存の範囲。
- 選択に応じた生成READMEの手順・説明。
- `init` で選んだ言語をどこに保持し、後続の `generate stanza` にどう反映するか。
- 既存リポジトリに選択情報がない場合と、非対話CLIでの指定方法・既定動作。

これらは検討項目であり、特定のオプション名、保存先、自動変換や混在対応を確定するものではない。
