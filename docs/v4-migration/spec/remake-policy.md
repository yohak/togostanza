...
現行版調査の生メモはここに記録しない。それらは `docs/v4-migration/investigation/` に置く。
...
現行版の観測結果そのものは `docs/v4-migration/investigation/` に置き、この文書ではリメイク版での判断だけを扱う。実装が進み、リメイク版単体の外部契約として確定した内容は、[リメイク版仕様](./index.md) へ移す。
...
非採用になる可能性が高い改善案も、判断材料として `docs/v4-migration/investigation/follow-ups.md` に残す。
