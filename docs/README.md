...
- [開発用ノート](./development-notes.md): 作業時の考え方や標準手順から外しやすい落とし穴。
...
3. 作業手順を書く前に [開発用ノート](./development-notes.md) を読む。
4. ローカルリポジトリの準備やCLI確認を始める前に [セットアップ](./setup/index.md) を読む。
5. 既存挙動調査を始める前に [既存挙動調査](./investigation/README.md) を読む。
6. リメイク版の判断を行うときだけ [リメイク方針](./spec/remake-policy.md) を使う。
7. 実装仕様として確定した内容を整理するときに [リメイク版仕様](./spec/index.md) を使う。
