...
localでは `develop` branchを作成し、`dist/` をGit追跡対象から外した。タグ無しGitHub dependencyはdefault branchを解決するため、公開remoteのdefault branchは `main` として維持する。GitHubのdefault branch確認・固定、branch protection、`develop` から `main` への反映方法は未実装であり、後続作業で定義する。
