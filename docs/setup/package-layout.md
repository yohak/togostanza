...
localでは `develop` branchを作成し、`dist/` をGit追跡対象から外した。タグ無しGitHub dependencyはdefault branchを解決するため、公開remoteのdefault branchは `main` として維持する。公開remoteのdefault branchが `main` であることは2026-07-27に確認した。remote `develop` のpush、branch protection、`develop` から `main` への反映実行は後続作業で扱う。
