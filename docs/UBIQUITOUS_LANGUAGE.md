...
| **検証環境** | 検証ケースごとに置く、現行版またはリメイク版のCLIを実行するためのStanza群プロジェクト | 検証プロジェクト、検証ケースパッケージ | `workbench/cases/<case>/current/`、`workbench/cases/<case>/remake/` | 確認済み |
...
- **検証環境** は、各 **検証ケース** の `current/` と `remake/` に分かれる。
- **検証ケース** では、同じケース内の両方の **検証環境** に対して同じ **CLI挙動** や runtime 挙動を確認する。
...
- **検証ケース** を、将来より具体的に「CLI検証ケース」「runtime検証ケース」などへ分けるべきか。
