...
| 診断メッセージの整理 | [follow-ups](../../investigation/follow-ups.md), [Phase 2 handoff](../phase-2/handoff.md), [Phase 3 handoff](../phase-3/handoff.md), [Phase 4 handoff](../phase-4/handoff.md) | 個別のvalidation、config、build、serve診断は増えているが、エラー/警告に対象Stanza ID、ファイルパス、修正内容をどの程度含めるかは統一していない。metadata schema validationやCLI exit code分類とは別の改善項目。 | `proposed phase` | 移行時とデバッグ時の自己解決性に直接影響する。診断が曖昧だと、仕様が正しくても利用者が修正箇所へ到達しにくい。 | developer experience / diagnosticsとして独立して扱う。Phase 5では文言やフォーマットを固定しない。 | Phase 7 candidate |
