...
棚卸し項目は、状態を示す `status` と、扱う候補を示す `phase` に分けて読む。
...
`phase` は仮置きである。複数候補を併記してよい。Phase 5では詳細APIや実装順序を固定しない。
...
| 診断メッセージの整理 | [follow-ups](../../investigation/follow-ups.md), [Phase 2 handoff](../phase-2/handoff.md), [Phase 3 handoff](../phase-3/handoff.md), [Phase 4 handoff](../phase-4/handoff.md) | 個別のvalidation、config、build、serve診断は増えているが、エラー/警告に対象Stanza ID、ファイルパス、修正内容をどの程度含めるかは統一していない。metadata schema validationやCLI exit code分類とは別の改善項目。 | `open` | 移行時とデバッグ時の自己解決性に直接影響する。診断が曖昧だと、仕様が正しくても利用者が修正箇所へ到達しにくい。 | developer experience / diagnosticsとして独立して扱う。Phase 5では文言やフォーマットを固定しない。 | Phase 7 candidate |
...
| Runtime edge semantics | [Phase 2-3 handoff](../phase-2/phase-2-3/handoff.md), [004](../../../workbench/cases/004-runtime-parameters/README.md), [005](../../../workbench/cases/005-stanza-source-api/README.md) | `this.params`、`render()`、`renderTemplate()`、`handleAttributeChange()`、`query()`、`importWebFontCSS()`、`menu()` の主要経路は成立。JSON parse失敗、invalid number / date / datetime、async renderの再入制御、render例外UI、`importWebFontCSS()` 重複抑止、menu DOM / keyboard interaction、`oldValue` / `newValue` の厳密観測は未固定。 | `open` | 細部を固定しすぎると互換面が広がるが、実アプリのエッジ挙動やデバッグ体験には影響する。 | 主要経路はdone、edge semanticsはsource API hardeningとしてまとめて扱う。個別にその場で仕様化しない。 | Phase 8 candidate |
...
11. 診断メッセージに対象Stanza ID、ファイルパス、修正内容をどこまで含めるか。

## Proposed roadmap seed

このロードマップは仮置きであり、Phase 5完了後の詳細計画で再確認する。

| phase | theme | candidate items |
| ---- | ---- | ---- |
