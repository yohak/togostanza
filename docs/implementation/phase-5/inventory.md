...
| Phase 7 candidate | developer experience and diagnostics | metadata validation、diagnostics整理、`init .` merge UX、`scripts.build` / `packageManager` field、serve preview改善 |
| Phase 8 candidate | compatibility expansion if needed | `stanza:include`、root asset参照API、Stanza間連携高度化、追加 `togostanza-utils` API、React / Vue version範囲 |
