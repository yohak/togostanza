...
| リッチなヘルププレビュー | Phase 13で、`build` と `serve` が `index.html`、`{id}.html`、ヘルププレビューappを生成・配信するようにした。パラメーター変更UI、style変更UI、HTML snippet、About、Download JSON導線を確認済み。 | 完了済み | Stanza開発者がローカル確認や公開済み生成物からstanzaを調整する導線は復元された。 | 残る改善はREADME本文表示、JSON専用editor、イベント操作UI、menu UI polishとして別項目で扱う。 |
| menu UI polish | Phase 2で最小menu shellは成立しているが、現行版の `<togostanza--menu>` custom element、info icon、popup、Copy HTML snippet、`togostanza-utils` のdownload系menu helper確認は未復元。 | 次にやる候補 | 埋め込み後のmenu挙動はStanza利用者とStanza開発者の体験に直接影響し、metastanzaでもdownload系menu helperが使われている。 | Phase 14で扱う。 |
