...
| リッチなヘルププレビュー | `build` と `serve` は最小プレビューを提供しているが、現行版相当のパラメーター変更UI、style変更UI、HTML snippet、About表示は未実装。 | 次にやる候補 | Stanza開発者がローカル確認や公開済み生成物からstanzaを調整しづらい。現行版からの開発体験の劣化に見える。 | Phase 13で、Vue 3とBootstrap 5 CSSを使ったヘルププレビューとして扱う。 |
