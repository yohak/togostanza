...
ローカルtarballを作って `npm install` / `pnpm install` できることは、Phase 1の完了条件にしない。実インストール確認はPhase 5のdistribution計画または配布前検証へ送る。
...
Phase 1では、実インストールは完了条件にしない。ローカルtarballを作って `npm install` / `pnpm install` できること、`node_modules/` が生成されること、lockfileが実install結果として生成されること、生成リポジトリ内で `npm exec togostanza ...` / `pnpm exec togostanza ...` が実際に解決されることは、Phase 5 distributionまたは配布前検証へ送る。
...
| ローカルtarballを使ったnpm/pnpmの実インストール確認 | Phase 5 / 配布前検証 |
| npm公開metadata、`exports`、`files`、`private` 解除 | Phase 5 |
